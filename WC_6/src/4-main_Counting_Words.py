...
if __name__ == '__main__':
    ...
    keywords_source = source_path.joinpath('Keywords List_2.xlsx')
    keywords=pd.read_excel(keywords_source)
    keywords['keyId']=keywords['Language']+'-'+keywords['Keyword Original']
    ...
    word_count_final=(word_count_df.T
        ...
        .merge(keywords[['keyId','Language','Keyword Original','Category','Keyword English']],how='inner',left_on='keyword',right_on='keyId')
        .set_index(['Category','Keyword English'])
    ...
    last_6_columns=final_reorganized.iloc[:,-6:]
    remaining_columns=final_reorganized.iloc[:,:-6]
    export_df = pd.concat([last_6_columns, remaining_columns], axis=1)
    export_df=export_df[export_df['Project']!=0]
    
    columns_6_to_end = [f"{level1}-{level2}" for level1, level2 in export_df.columns[6:]]
    columns_1_to_6 = list(export_df.columns[:6])
    new_columns=columns_1_to_6+columns_6_to_end
